...
function FaviconImg({ name }: { url: string; name: string }) {
  return (
    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-[11px] font-bold text-slate-500">
      {name.charAt(0)}
    </div>
  );
}
  ...
  return (
    <div className="py-8">
      ...
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          <p className="text-[11px] font-medium uppercase tracking-wider text-slate-400">Searching for</p>
          <p className="text-[16px] font-semibold text-slate-900">"{query}"</p>
        </div>
      </div>
      ...
      <div className="mb-8 space-y-2">
        {searchSteps.map((step, i) => {
          ...
          return (
            <div
              key={i}
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-[13px] transition-all duration-300 ${
                isActive
  ...
  return (
    <div className="py-6">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <p className="text-[11px] font-medium uppercase tracking-wider text-slate-400">Results for</p>
          <p className="text-[15px] font-semibold text-slate-900">"{query}"</p>
        </div>
      </div>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                <div>
                  <a
                    ...
                    className="text-[17px] font-bold text-slate-900 no-underline hover:text-blue-600"
                  >
                    {best.name}
                  </a>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {best.purposes.map((p) => (
                  <span
                    key={p}
                    className="rounded-md bg-violet-50 px-2 py-0.5 text-[10px] font-medium text-violet-700"
                  >
                    {p.replace('-', ' ')}
                  </span>
                ))}
              ...
              <div className="mt-4 flex items-center gap-2">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-[12px] font-medium text-white no-underline transition-colors hover:bg-blue-700"
                >
                  Visit Website
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-4 py-2 text-[12px] font-medium text-slate-700 no-underline transition-colors hover:bg-slate-50"
                >
                  More Details
      ...
      {result.interpretedTags.length > 0 && (
        <div className="mb-6 flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-medium text-slate-400">Interpreted as:</span>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[13px] font-semibold text-slate-900 no-underline hover:text-blue-600"
                    >
                      {site.name}
                    </a>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center">
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 bg-slate-50/80 px-5 py-3">
          ...
          <span className="text-[12px] font-semibold text-slate-700">AI Search</span>
          <div className="flex-1 truncate rounded-md border border-slate-200 bg-white px-3 py-1 text-[13px] text-slate-600">
            {query}
          </div>
          {phase === 'processing' && (
          ...
          <div className="px-5">
            ...
            {phase === 'results' && !result && (
              <div className="py-12 text-center">
                <p className="text-[14px] font-medium text-slate-500">No results found for "{query}"</p>
